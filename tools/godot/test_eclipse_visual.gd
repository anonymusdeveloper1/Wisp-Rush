extends SceneTree
## Eclipse (whole-frame sprite): the pack contract, state and wall mapping, the menu
...
##
##   Godot --headless --path . --script res://tools/godot/test_eclipse_visual.gd
...
const FORM_ID: StringName = &"eclipse"
const GAMEPLAY_FRAMES: String = "res://data/characters/eclipse_gameplay.tres"
const MENU_FRAMES: String = "res://data/characters/eclipse_menu.tres"
## How much faster than the pack authored them the looping animations actually play.
## Eclipse's pack plays at the rate it was authored at; there is no phone workaround on it.
...
func _run() -> void:
	...
	if _failures > 0:
		push_error("eclipse_visual: %d failures" % _failures)
		quit(1)
		return
	print("eclipse_visual: pack, states, walls, menu, split and reduced motion passed")
	quit()
...
func _check_game_world() -> void:
	...
	var visual: EclipseVisual = null
	if player != null:
		visual = player.get_character_visual() as EclipseVisual
	if visual == null:
...
func _fail(message: String) -> void:
	_failures += 1
	push_error("eclipse_visual: %s" % message)


func _spawn(menu: bool = false) -> EclipseVisual:
	...
	if form == null or form.visual_scene == null:
		...
		return null
	var visual := form.visual_scene.instantiate() as EclipseVisual
	if visual == null:
		_fail("the %s scene is not a EclipseVisual" % FORM_ID)
		return null
...
func _settles_on(
	visual: EclipseVisual, state: StringName, animation: StringName, frames: int = 180
) -> bool:
...
func _check_pack_contract() -> void:
	...
	var sheet: CharacterPoseSheet = load("res://data/characters/eclipse_frames.tres")
	if sheet == null:
...
func _check_states() -> void:
	var visual: EclipseVisual = _spawn()
	if visual == null:
		return
	await process_frame
...
func _check_menu() -> void:
	var visual: EclipseVisual = _spawn(true)
	if visual == null:
		return
	await process_frame
...
func _check_frame_set_split() -> void:
	var playing: EclipseVisual = _spawn()
	var menu: EclipseVisual = _spawn(true)
