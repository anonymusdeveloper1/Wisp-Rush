...
func _add_panels() -> void:
	...
	var shards := _panel(&"PanelPlate")
	var shard_row := HBoxContainer.new()
	var shard_icon := TextureRect.new()
	shard_icon.texture = load(ICON_DIR + "02_soul_shards.png")
	...
	var shard_value := _label("320", &"ValueLabel")
	shard_value.add_theme_font_size_override(&"font_size", 40)
	shard_row.add_child(shard_value)
	shards.add_child(shard_row)
	plates.add_child(shards)
