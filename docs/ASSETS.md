...
| `res://assets/art/characters/playable/<id>/` (veyra 12, rook 13, morrow 12, bram 17, ilyra 67 + `manifest.json`) | Layers of the five playable characters plus each one's `preview.png` portrait (HUD, Home and card art) | `python3 tools/art/extract_playable_characters.py` — fixed cells from a rig source for veyra/rook/morrow/bram (`SOURCE_SHEETS`), or a verbatim copy of a per-file part pack for ilyra (`PART_PACKS`) | generated art; licence to confirm before release (GDD §14 #1, #21) | transparent RGBA, sizes as the cell crops; lossless import (no mipmaps, like the forms); referenced by the rig scenes in `scenes/player/visuals/`; never hand-edit — re-run the extractor and paste the printed ribbon spines | 2026-09-17 |
