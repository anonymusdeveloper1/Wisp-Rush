...
| `concept_art/wisp_rush_character_candidates_v1/` | approval sheets for Legendary Bram and Mythic Nymera | Codex built-in OpenAI image generation from `GENERATION_PROMPTS.md`, using the owner's Morrow/Rook concepts as style-only references | generated art; licence to confirm before release | concept-only RGB PNGs under `concept_art/.gdignore`; not runtime art and not approved for catalog integration | 2026-09-18 |
