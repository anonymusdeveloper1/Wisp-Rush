...
_Last updated: 2026-09-15 · by: Claude Code (Opus 5)_
...
- **Phase:** Redesign v1 integrated (ADR-0005/0006); runs on the owner's Android phone from a debug APK.
- **Works:** Loading → Tutorial screen (first launch) → animated Home → Rift story or Endless runs with five Rifts, twists and bosses →
  Results with a Rift Points breakdown; Rift Map, Trials, Daily, a four-tab RP Shop (No Ads disabled); save v6; synthesized
  audio; Reduced Motion; Android back. 33 headless test scripts (`tools/run_tests.sh`).
- **Direction settled 2026-09-14 ([ADR-0013](decisions/0013-rift-story-levels-endless-mode-and-rift-points.md)):**
  Rifts become finite story levels, a cosmetic Endless mode joins them on one floor template
  ([ADR-0014](decisions/0014-endless-arenas-share-one-floor-template.md)), Soul Shards become earned-only
  Rift Points and the Sanctum goes. Build order: [specs/story_and_endless/](specs/story_and_endless/README.md) (ROADMAP M10).
- **Next:** spec 02 (story levels); spec 01 (Rift Points, Sanctum removed) landed 2026-09-15; owner
  generates the Endless skins; RP pacing waits for a device pass. RUSH and fast feel (M11) built
  2026-09-15; device pass pending. Tutorial screen (M12) built 2026-09-15; device pass pending.
...
| Rifts | ✅ done · story-level rework planned (ADR-0013) | [rifts.md](systems/rifts.md) | `res://scenes/screens/rift_map_screen.tscn` |
| Endless mode | 🔄 built on a placeholder skin (spec 03) · real skins in spec 05 | [endless_mode.md](systems/endless_mode.md) | `res://data/endless/default_endless_catalog.tres` |
...
| `EconomyTuning` | `res://scripts/resources/economy_tuning.gd` | `res://data/economy/default_economy_tuning.tres` | Rift Points payouts that are not pickups: `score_per_rift_point` performance bonus (spec 02 adds clear bonuses) |
...
| Endless | The never-ending mode planned by ADR-0013, on cosmetic arena skins over one shared floor |
