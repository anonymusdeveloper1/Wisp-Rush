...
Fragments are found, not granted: a killed enemy rarely drops a **Soul Vessel**, which adds one
fragment and fills it. There is no ceiling — a long run can carry far more than the three a run used
to start with. Reaper's Gift still heals a missing fragment on a kill streak but never raises the
maximum.
...
- Spent only in the Shop. There is **no permanent power**: the Soul Sanctum is removed, and permanent
  progress is content access and cosmetics. Run-only power is the seven mutations plus the Soul
  Vessels a run finds.
...
| Tab | Items | Paid with |
|---|---|---|
| Characters | Void (default Wisp form), Eclipse (2,000 plus first boss victory). Animated characters: Veyra, the Last Wisp; Rook, the Bonewing; Morrow, the Runebound; Noxen, the Veilflame (Legendary). Whole-frame sprite characters: Ilyra, the Astral Dancer (Mythic); Shade (Standard, **placeholder tier and price**). Card order, owner 2026-09-20: Void, Eclipse, Veyra, Noxen, Shade, Ilyra, Morrow, Rook. Every character plays exactly like the Wisp (rigs 0 while the owner reviews them, §14 #31). Roster cut to seven on 2026-09-20 (§14 #33) | RP |
...
- **Shop** has three tabs: Characters, Arenas (Endless skins) and No Ads (Dashes was removed
