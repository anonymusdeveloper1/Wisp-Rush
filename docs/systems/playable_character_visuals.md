...
> **Status:** ✅ Veyra, Rook and Morrow built 2026-09-17 · owner approval and device pass pending ·
> **Last updated:** 2026-09-17 · **GDD section:** §6, §9, §11, §14 #31 ·
...
Bonewing; and Morrow, the Runebound join the six single-image Wisp forms in the Shop's CHARACTERS
tab. Collision, health, dash, scoring, controls and camera never change with the character.
...
| `res://scenes/player/visuals/veyra_visual.*` · `rook_visual.*` · `morrow_visual.*` | One rig per character |
| `res://assets/art/characters/playable/<id>/` | Runtime layers (generated, never hand-edit; [ASSETS.md](../ASSETS.md)) |
| `res://data/forms/veyra.tres` · `rook.tres` · `morrow.tres` | Identity, portrait, price 0, tint, `visual_scene` |
| `tools/art/extract_playable_characters.py` | Layers + ribbon spines from `concept_art/wisp_rush_playable_characters_v1/assets/` |
...
- `tools/run_tests.sh form_catalog` — nine characters, prices, rigs, save ids.
...
- Owner approval of the look and motion, then prices (all three cost 0 RP for the review).
- Not yet seen on a phone: readability at gameplay size, particle cost, 120 Hz feel (position still
  steps at the 60 Hz physics rate).
