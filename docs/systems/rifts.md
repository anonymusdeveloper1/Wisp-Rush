...
> **Status:** ✅ done (ladder, levels, difficulty, rosters, all four rule twists and five bosses) ·
> **Last updated:** 2026-09-13 · **GDD section:** §5.4, §6 · **ADR:** [0007](../decisions/0007-rifts-as-rule-variant-arenas.md)

## Purpose

Give the endless run a ladder of arenas that each change one rule, with a map screen that shows the
progression, the rule twist, the unlock gate and the personal best per arena.

## Files

| Path | Role |
|---|---|
| `res://scripts/resources/rift_data.gd` | One arena: identity, backdrop, unlock gate, accent, rule key |
...
| `RiftData.is_unlocked(highest_wave)` | method | Whether a lifetime best wave clears the gate. |
...
| `GameWorld.configure_run_profile(form, daily_date, rift)` | method | Third argument supplies the active Rift. |

## Data & tuning

| Rift | Gate | Tier | `rule_key` | Twist | L1 → L8 threat | Speed | Boss |
|---|---|---|---|---|---|---|---|
| Obsidian Garden | wave 0 | 1 | `none` | Baseline arena | 1.00 → 1.84 | ×1.00 | `reaper` |
| Shattered Rift | wave 5 | 2 | `portals` | Void portals bend the dash to their pair | 1.00 → 2.05 | ×1.05 | `the_fracture` |
| Ember Hollow | wave 10 | 3 | `shrinking_floor` | The floor burns away as the run goes on | 1.00 → 2.26 | ×1.10 | `cinder_maw` |
| Frozen Choir | wave 15 | 4 | `drift` | Ice keeps the Wisp drifting after impact | 1.00 → 2.47 | ×1.15 | `hollow_choir` |
| Reaper's Court | wave 20 | 5 | `boss_rush` | Reaper every two waves, double rewards | 1.00 → 2.75 | ×1.20 | `reaper_ascended` |
...
per-level ramp, which steepens with the tier. Levels advance *within* a run on each boss defeat and
persist as `rift_levels`.
...
- **Unlocking is derived from the lifetime `highest_wave`, never stored**, so it cannot desync.
...
- **Map UI (owner decision 2026-09-13):** a portrait carousel like Forms. Swipe or tap a side card
  ...
  - Unlocked card: arena art, name, `TIER n`, `LEVEL n / 8` (or `ALL 8 CLEARED  ·  ENDLESS`) and a
    slim level bar. Locked card: dimmed art, a lock and `REACH WAVE N`.
  - Under the carousel: dots (hollow = locked), the rule summary, a status line — amber `BEST n`,
    `NO RUN YET`, or `LOCKED  ·  YOUR BEST WAVE x OF N` — then BACK and ENTER.
...
- Permanent upgrades live in the Soul Sanctum ([meta_progression.md](meta_progression.md)) and are
  gated by shards and prerequisites, **not** by Rift level progress.
...
- Per-Rift mission chains (ROADMAP M8) are not built; unlocking is wave-gated only.
