...
- **MCP:** server `godot` from `.mcp.json`, approved in `.claude/settings.json`. Its tools appear as
  `mcp__godot__<tool>` (tool guide: AGENTS.md §4). Video work uses `palmier-pro` (Palmier Pro's own
  server, registered in the local Claude config; the app must be open) — `tools/video/README.md`.
