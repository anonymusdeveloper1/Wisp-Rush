#!/usr/bin/env python3
"""Build the Eclipse Wisp whole-frame review pack from generated phase atlases.
...
FRAMES = ROOT / "frames" / "eclipse_wisp"
SHEETS = ROOT / "sheets"
...
def strip_top_edge_bleed(image: Image.Image) -> Image.Image:
    """Drop clipped pieces inherited from the atlas cell above.

    Generated atlases sometimes place a severed flame tip on the first pixel row of the next cell.
    Legitimate eclipse beads remain: a component is removed only when it touches the top edge and
    lies wholly above the largest connected component (the Wisp body).
    """
    pixels = np.asarray(image.convert("RGBA")).copy()
    solid = pixels[:, :, 3] > 8
    occupied_rows = solid.any(axis=1)
    occupied = np.nonzero(occupied_rows)[0]
    if occupied.size and occupied[0] <= 2:
        # A clipped neighbour can remain one anti-aliased component because of edge noise. A
        # clearly empty horizontal band between it and the real pose is stronger evidence.
        last_edge_row = int(occupied[0])
        while last_edge_row + 1 < len(occupied_rows) and occupied_rows[last_edge_row + 1]:
            last_edge_row += 1
        next_rows = occupied[occupied > last_edge_row]
        if next_rows.size and int(next_rows[0]) - last_edge_row >= 5:
            pixels[: int(next_rows[0])] = 0
            solid = pixels[:, :, 3] > 8
    ...
    body_top = int(np.nonzero(labels == body)[0].min())
    remove: list[int] = []
    for component in range(1, count + 1):
        if component == body:
            continue
        rows = np.nonzero(labels == component)[0]
        if rows.min() <= 2 and rows.max() < body_top:
            remove.append(component)
    if remove:
        pixels[np.isin(labels, remove)] = 0
...
def extract_cell(atlas: Image.Image, index: int, canvas_size: int) -> Image.Image:
    ...
    cell = strip_top_edge_bleed(atlas.crop((x0, y0, x1, y1)))
...
def make_preview(sheet: Image.Image, manifest: dict[str, object]) -> Image.Image:
    ...
    preview = Image.new("RGB", (label_width + REVIEW_COLUMNS * cell, len(rows) * cell), (14, 8, 25))
    ...
    for row_info in rows:
        ...
        y = row * cell
        draw.rectangle((0, y, preview.width - 1, y + cell - 1), outline=(74, 44, 104))
        draw.text((16, y + 36), state, fill=(255, 237, 202), font=font)
        draw.text((16, y + 66), f"{frame_count} frames", fill=(216, 169, 255), font=small)
        ...
        for column in range(REVIEW_COLUMNS + 1):
            ...
            draw.line((x, y, x, y + cell), fill=(52, 30, 74), width=1)
    return preview
...
def make_loop_preview(state: str, images: list[Image.Image]) -> None:
    ...
    for image in images:
        ...
        background = Image.new("RGB", cell.size, (14, 8, 25))
...
def main() -> None:
    ...
    manifest["qa"] = qa
    sheet.save(SHEETS / "eclipse_wisp_sprite_sheet.png", optimize=True)
    make_preview(sheet, manifest).save(SHEETS / "eclipse_wisp_sprite_sheet_preview.png", optimize=True)
    (SHEETS / "eclipse_wisp_sprite_sheet.json").write_text(
